...
from Gnomix.Base.models import LogisticRegressionBase, CovRSKBase
from Gnomix.Smooth.models import XGB_Smoother
from Gnomix.Gnofix.gnofix import gnofix
...
class Gnomix():
    ...
    def __init__(self, C, M, A, S,
        ...
        if smooth is None:
            if mode == "fast":
                from Gnomix.Smooth.models import CRF_Smoother # import here to avoid strict crf suite dependency
                smooth = CRF_Smoother 
            elif mode == "large":
                from Gnomix.Smooth.models import CNN_Smoother # import here to avoid strict torch dependency
                smooth = CNN_Smoother 
            elif mode=="best":
                smooth = XGB_Smoother
            else:
    ...
    def write_config(self,fname):
        with open(fname,"w") as f:
            for attr in dir(self):
                ...
                if type(val) in [int,float,str,bool,np.float64,np.float32,np.int32]:
                    f.write("{}\t{}\n".format(attr,val))
